...
function setup()
{
    ...
    if (!xvp_db_user_has_rights($vm, null, "all"))
	return;
...
function no_hosts()
{
    ...
    echo "<p class=\"action\">No suitable hosts on which to boot $htvmname</p>";
}

function choose_host()
{
    global $poolname, $vmname, $home, $hosts;
...
function boot_vm()
{
    global $session, $ref, $href, $booted;
...
function main()
{
    global $stage, $hosts, $booted, $onload;
...
EOF;
    ...
    echo <<<EOF
<body onload="window.parent.actionCallback('$jsvmname', 'boot', '$booted');">
