...
function pool_display($pool)
{
    ...
    $displayname = htmlentities($pool->poolname, ENT_QUOTES);
    if ($pool->domainname)
	$displayname .= " in domain " . htmlentities(substr($pool->domainname, 1), ENT_QUOTES);
...
function vm_display($vm, &$groupname)
{
	...
	$displayname = htmlentities($groupname, ENT_QUOTES);
	$groupclass = xvp_make_fullname($poolname, $groupname);
