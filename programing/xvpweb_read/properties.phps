...
function main()
{
    ...
    global $change_dvd, $dvd_uuid, $dvd_error;
    ...
    switch (xvp_db_user_rights($vm, null)) {
    case "none":
    case "list":
	return;
    case "all":
    ...
    if (!xvp_db_user_has_rights($vm, null, "read"))
	return;
    ...
    $dvd_error = "";
    if ($change_dvd && isset($dvd_uuid) && $dvd_uuid != $current_dvd_uuid) {
	if ($dvd_uuid == "empty") {
	...
	// grubby hack for Windows(R) etc
	$vm->os_version = str_replace("\xae", "(R)", $osversion);
    }
...
function show_dvd_drive()
{
    ...
    global $current_dvd_uuid, $current_dvd_name, $possible_dvds, $change_dvd;
...
EOF;
	...
	$selected = ($uuid == $current_dvd_uuid) ?
	    " selected=\"selected\"" : "";
	echo "    <option value=\"$uuid\"$selected onclick=\"document.forms[0].submit();\">$htname</option>\n";
    }
